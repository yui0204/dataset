...
class Wavedata:
    ...
    def cut_wav(self, start_sec, end_sec, window=False):
        ...
        if window == True:
            window = np.hanning(len(cut_sound))
            cut_sound = cut_sound * window
        
        return Wavedata(self.__fs, cut_sound, self.__name, self.__nbyte)   
    
    
    def zero_padding(self, total = 131072 + 256, start_idx=0):
        """このメソッドは指定した秒数部分だけを抽出し、Wavedataクラスを返します
    ...
    def synthesis(self, wavedata, synthesis_name = "synthesis"):
        ...
        if judge == True:
            if len(self.norm_sound) <= len(wavedata.norm_sound):
                ...
                synthesis_sound = self.norm_sound[0:len(wavedata.norm_sound)] \
                                + wavedata.norm_sound
